...
void exec_root_shell(int client, int pid, SuRequest &req, MntNsMode mode) {
    ...
    if (!req.context.empty()) {
        // Try an immediate dynamic transition (setcon) first so the target
        // domain is active right away; fall back to the legacy exec-time
        // transition if the policy rejects setcon.
        auto f = xopen_file("/proc/self/attr/current", "we");
        if (!f || fprintf(f.get(), "%s", req.context.c_str()) < 0) {
            f = xopen_file("/proc/self/attr/exec", "we");
