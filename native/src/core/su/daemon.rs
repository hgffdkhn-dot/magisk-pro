...
use crate::daemon::{AID_INET, AID_ROOT, AID_SHELL, AID_SYSTEM, MagiskD, to_app_id, to_user_id};
            ...
            if req.target_uid == AID_ROOT && req.gids.is_empty() && req.context.is_empty() {
                match access.settings.group {
                    1 => {
                        ...
                        // gid plus inet so networking keeps working.
                        req.target_uid = AID_SYSTEM;
                        req.gids.push(AID_SYSTEM as u32);
                        req.gids.push(AID_INET as u32);
                        req.context = "u:r:system_app:s0".into();
                    }
                    2 => {
                        // Run as the adb shell (u:r:shell:s0) with the shell
                        // primary gid plus inet.
                        req.target_uid = AID_SHELL;
                        req.gids.push(AID_SHELL as u32);
                        req.gids.push(AID_INET as u32);
                        req.context = "u:r:shell:s0".into();
                    }
                    _ => {}
                }
            }
