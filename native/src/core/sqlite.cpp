...
sqlite3 *open_and_init_db() {
    ...
    auto create_policy = [&] {
        return sql_exec_impl(db.get(),
                ...
                "notification INT, grp INT NOT NULL DEFAULT 0, "
                "PRIMARY KEY(uid))");
    };
    ...
    // 13: add column `grp` to table `policies` (per-app permission group)
    ...
    if (ver == 11) {
        sql_chk_log(sql_exec_impl, db.get(),
                ...
                "notification INT, grp INT NOT NULL DEFAULT 0, "
                "PRIMARY KEY(uid));"
                "INSERT INTO policies (uid, policy, until, logging, notification) "
                "SELECT uid, policy, until, logging, notification FROM policies_tmp;"
                "DROP TABLE policies_tmp;"
                "COMMIT;");
        // The new table already contains `grp`, skip the ver == 12 ALTER
        ver = 13;
        upgrade = true;
    }
    if (ver == 12) {
        sql_chk_log(sql_exec_impl, db.get(),
                "ALTER TABLE policies ADD COLUMN grp INT NOT NULL DEFAULT 0;");
        ver = 13;
        upgrade = true;
    }
