...
impl SePolicy {
    pub fn magisk_rules(&mut self) {
        ...
        rules! {
            ...
            // Let su requests dynamically transition into app/shell domains
            allow([proc], [proc], ["process"], ["setcurrent"]);
            allow([proc], ["system_app", "shell"], ["process"], ["dyntransition"]);

            // After setcon, the su process needs to exec the shell binary
            // without a type_transition (staying in the target domain)
            allow(["system_app", "shell"], ["shell_exec"], ["file"], ["execute", "execute_no_trans"]);
